...
final class Square {
    ...
    /** KING_SQUARES[i][d] is a list of all squares one rook move in direction
     *  d from the king square with index i. Direction displacements are defined
     *  by DIR, above. Lists are in order of increasing distance from
     *  square i. */
    static final SqList[][] KING_SQUARES = new SqList[SQUARES.length][4];

    static {
        for (Square sq : SQUARES) {
            int r0 = sq.row(), c0 = sq.col(), i0 = sq.index();
            for (int d = 0; d < DIR.length; d += 1) {
                KING_SQUARES[i0][d] = new SqList();
                SqList L = KING_SQUARES[i0][d];
                int c1 = c0 + DIR[d][0], r1 = r0 + DIR[d][1];
                if (!exists(c1, r1)) {
                    break;
                }
                L.add(sq(c1, r1));
